...
// Motor Variables
float Theta_curr,Omega_curr, RPM_curr, RPM_Desired, RPM_curr_filtered, RPM_prev_filtered ;
float Theta_prev = 0;
float RPM_max = 230;
float dt;
const float Ts = 0.01;
...
void loop(){
  ...
  if(localCount > count_prev){
    ...
    e = RPM_Desired - RPM_curr_filtered; // Error Calculation

    float V_unsat = kp*e + ki*inte_prev + kd*(e-e_prev)/dt;

    float V_sat = constrain(V_unsat,Vmin,Vmax);
    
    inte = inte_prev + ((dt) * (e + e_prev)/2) +(dt/T_tracking)*(V_sat - V_unsat); // Integral Error Calculation
    
    V = kp*e + inte*ki + (kd * (e - e_prev)/(dt)); // Control input calculation
